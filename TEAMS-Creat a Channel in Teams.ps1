...
# Type: New Service
#
# Title: M365 - Create a Channel in Teams
#
# v 1.0 Initial Release
#
# Init Release: 10.02.2022
# Last Update: 10.02.2022
# Code Template V 1.3
#
# Github: https://github.com/au2mator/TEAMS-Create-a-Channel-in-Teams
#################
...
param (
    ...
    [parameter(Mandatory = $true)] 
    [String]$c_ChannelName,

    [parameter(Mandatory = $true)]
    [String]$c_ChannelPrivacy,

    [parameter(Mandatory = $true)]
    [String]$c_PrivateChannelOwner,
...
[string]$LogPath = "C:\_SCOworkingDir\TFS\PS-Services\M365 - Create a Channel in Teams"
[string]$LogfileName = "Create a Channel in Teams"
...
try {
    ...
    try {
        Write-au2matorLog -Type INFO -Text "Try to create the Channel in Team"

        $URL = "https://graph.microsoft.com/v1.0/teams/$($c_TeamID)/channels" 
        if ($c_ChannelPrivacy -eq "Standard")
        {
            Write-au2matorLog -Type INFO -Text "Build JSON for Standard Channel"

        $BodyJsonChannel = @"
            {
               
               "displayName":"$c_ChannelName",
               "description":"Created with au2mator",
               "membershipType":"$c_ChannelPrivacy"
...
"@
            ...
            Write-au2matorLog -Type INFO -Text "Build JSON for Private Channel"
            $TempOwner = (Get-ADUser -Identity $c_PrivateChannelOwner).UserPrincipalName
            ...
            "members":
                [
                    ...
                    "user@odata.bind":"https://graph.microsoft.com/v1.0/users('$TempOwner')",
                    "roles":["owner"]
                    }
                ]
            }
"@
        ...
        Write-au2matorLog -Type INFO -Text "Created the Channel"

        
    }
    catch {
        Write-au2matorLog -Type ERROR -Text "Could not create Channel"
        Write-au2matorLog -Type ERROR -Text "Error: $Error"

        $au2matorReturn = "Could not create Channel, Error: $Error"
        $AdditionalHTML = "Could not create Channel
        <br>
        Error: $Error
            "
        $Status = "ERROR"
